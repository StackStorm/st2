...
class TestWebhooksController(FunctionalTest):
    ...
    def test_st2_webhook_lower_header(self, dispatch_mock):
        data = WEBHOOK_1
        post_resp = self.__do_post('git', data,
                                   headers={'X-Github-Token': 'customvalue'})
        self.assertEqual(post_resp.status_int, http_client.ACCEPTED)
        self.assertEqual(dispatch_mock.call_args[1]['payload']['headers']['X-Github-Token'],
                         'customvalue')
        self.assertEqual(dispatch_mock.call_args[1]['payload']['headers_lower']['x-github-token'],
                         'customvalue')
