...
class PythonRunner(ActionRunner):
    ...
    def run(self, action_parameters):
        ...
        # Note: Temporary auth token is generated by the runner container service
        auth_token_db = getattr(self, 'auth_token', None)
        auth_token = getattr(auth_token_db, 'token', '')

        env[API_URL_ENV_VARIABLE_NAME] = get_full_public_api_url()
        env[AUTH_TOKEN_ENV_VARIABLE_NAME] = auth_token
