...
class TestActionExecutionHistoryModel(DbTestCase):
    ...
    def test_crud(self):
        ...
        escaped_trigger = util.escape_chars(copy.deepcopy(self.fake_history['trigger']))
        self.assertDictEqual(model.trigger, escaped_trigger)
        escaped_trigger_type = util.escape_chars(copy.deepcopy(self.fake_history['trigger_type']))
        self.assertDictEqual(model.trigger_type, escaped_trigger_type)
        escaped_trigger_in = util.escape_chars(copy.deepcopy(self.fake_history['trigger_instance']))
        ...
        escaped_action = util.escape_chars(copy.deepcopy(self.fake_history['action']))
        self.assertDictEqual(model.action, escaped_action)
        escaped_runner = util.escape_chars(copy.deepcopy(self.fake_history['runner']))
        self.assertDictEqual(model.runner, escaped_runner)
        escaped_execution = util.escape_chars(copy.deepcopy(self.fake_history['execution']))
