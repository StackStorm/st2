...
class TraceDBTest(CleanDbTestCase):
    ...
    def test_query(self):
        saved = TraceDBTest._create_save_trace(
            ...
            rules=[str(bson.ObjectId()) for _ in range(4)],
            trigger_instances=[str(bson.ObjectId()) for _ in range(5)])
        retrieved = Trace.query(trace_id=saved.trace_id)
        self.assertEquals(len(retrieved), 1, 'Should have 1 trace.')
        self.assertEquals(retrieved[0].id, saved.id, 'Incorrect trace retrieved.')

        # Add another trace with same trace_id and confirm that we support.
        # This is most likley an anti-pattern for the trace_id but it is an unknown.
        saved = TraceDBTest._create_save_trace(
            trace_id='test_trace',
    ...
    @staticmethod
    def _create_save_trace(trace_id, id_=None, action_executions=None, rules=None,
